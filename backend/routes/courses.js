...
const db = require('../db');
const {authenticate, authorizeRole, optionalAuth} = require('../middleware/auth');
...
router.patch(
	...
	async (req, res) => {
		try {
			...
			if (req.user.role === "teacher") {
				updates.teacher = req.user.id;
			}
			
			// ✅ Faqat ruxsat etilgan maydonlarni o‘tkazamiz
			const allowed = ["title", "description", "price_cents", "published", "preview_image", "category", "teacher"];
